...
def detect_landmarks(img0,face_box,img_size,model_landmark):


    #ckpt_file = './pretrained_model/blazelandmark.pth'
    image_size = 112  # 112 landmarks face image_size
    ...
    x1, y1, x2, y2 = int(face_box[0]+0.5),int(face_box[1]+0.5),int(face_box[2]+0.5),int(face_box[3]+0.5)
    w = x2 - x1 + 1
    h = y2 - y1 + 1
    ...
    input = input.half()  #  to fp16

    #cv2.rectangle(img0, (x1, y1), (x2, y2), (0, 255, 0))
    pre_landmarks, _ = model_landmark(input.cuda())
    ...
    #for (x, y) in pre_landmark.astype(np.int32):
    #    cv2.circle(img0, (x1 + x, y1 + y), 1, (0, 0, 255), 2)
    return x1,y1,pre_landmark
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if names[int(c)]=='face' :
                            ...
                            x1,y1,pre_landmark=detect_landmarks(im0,face_box,opt.img_size,model_landmark)

                        plot_one_box2(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=1,x1=x1,y1=y1,pre_landmark=pre_landmark)
